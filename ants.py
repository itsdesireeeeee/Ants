...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while temp_place != beehive:
            if temp_place.bees and self.min_range <= distance <=self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        for bee in list(place_copy.bees):
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting > 0:
            ...
            specific_bee = rANTdom_else_none(self.place.bees)
            if specific_bee:
                self.eat_bee(specific_bee)
...
# BEGIN Problem EC
class QueenAnt(ScubaThrower):  # You should change this line(CHANGED)
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        QueenAnt.possible_queen.append(self)
        if len(self.possible_queen) >= 2: #len ?
            QueenAnt.imposter = self   #make it an imposter queen 
    ...
    def action(self, gamestate):
        ...
        if self == QueenAnt.imposter:
            ...
            while temp_place is not None:
                if temp_place.ant and temp_place.ant != self:
                    if not temp_place.ant in self.ants_w_double:
                        temp_place.ant.damage *= 2
                        self.ants_w_double.append(temp_place.ant)
